...
pub async fn service_create_new_employee_by_user_exist(
    ...
    let new_employee_id = insert_new_employee(state, &tx, req).await;
...
pub async fn insert_new_employee(
    ...
    let user = UserEntity::find_data_by_uuid(
        &*state.db,
        &create_new_employee_by_user_uuid_request.user_uuid,
    )
    .await;
    if user.is_none() {
    ...
    let employee_insert = EmployeeEntity::insert(employee.into_active_model()).exec(tx).await?;
    ...
    let employee_department_model = employee_department::Model {
        ...
        department_id: department.unwrap().department_uuid,
        create_at: Utc::now().naive_utc(),
        update_at: Utc::now().naive_utc(),
    };
    ...
    let position = DepartmentEntity::find_data_by_uuid(
        &*state.db,
    ...
    let employee_position_model = employee_position::Model {
        employee_id: employee_insert.last_insert_id,
        position_id: position.unwrap().department_uuid,
        create_at: Utc::now().naive_utc(),
        update_at: Utc::now().naive_utc(),
    };

    let employee_position_insert =
        EmployeePositionEntity::insert(employee_position_model.into_active_model())
            .exec(tx)
            .await?;
    if employee_position_insert.last_insert_id.1 <= 0 {
        return Err(AppError::EntityNotAvailableError { detail: "Employee".to_string() });
    }
...
pub async fn service_create_new_employee(
    ...
    let create_new_employee_by_user_exist = CreateNewEmployeeByUserUuidRequest {
        ...
        department_uuid: Default::default(),
        position_uuid: Default::default(),
    };

    let employee_insert_id =
        insert_new_employee(state, &tx, &create_new_employee_by_user_exist).await?;
    if employee_insert_id <= 0 {
        return Err(AppError::EntityNotAvailableError { detail: "Employee".to_string() });
    };
    tx.commit().await?;
...
pub async fn service_update_employee(
    ...
    let a = req.get_department_uuid().clone().unwrap_or(vec![]);
    let departments =
        DepartmentEntity::find().filter(department::Column::DepartmentUuid.is_in(a)).all(&tx).await;
