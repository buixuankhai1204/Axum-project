...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let statement = db_postgres.build(&schema.create_table_from_entity(OrganizationEntity));
        db.execute_unprepared(statement.sql.as_str()).await?;
        Ok(())
    }

    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.get_connection().execute_unprepared("DROP TABLE IF EXISTS organizations").await?;
        Ok(())
    }
}
